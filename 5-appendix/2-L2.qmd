...
::: Definition
# Real Vector Space
A _vector space_ is a set $\mathcal V$ that satisfies the following properties: 
...
![Functions as vectors and vice-versa](vectors.jpg)
...
It's funny, but this kind of thinking is a staple of mathematics. We all use stupid pictures or imaginations grounded in our physical experince which we know are "wrong" in some way (e.g. 3 dimensions instead of 14). It's the only way to build intuition! The important thing is knowing that the pictures are wrong in some ways. But the more wrong pictures you have (e.g. from different "perspectives"), the better, since you can switch to a picture that "correctly" illustrates the phenomenon of interest.
:::
...
$$
\|f\|^2 = \int_{-\infty}^\infty f(x)^2 \ \text{d} x
$$

to be our norm. We call this the _$\mathcal L_2$ norm_ (there are other choices; the "2" is for the square inside the integral). Although there are other possible norms, the $\mathcal L_2$ norm is nice because it is compatible with an inner product, namely 
$$
\langle f, g \rangle = \int_{-\infty}^{\infty} f(x) g(x) \ \text{d} x
.
$$
...
::: Exercise
Take $f(z) = 1(z=0)$ and $g(z) = 0$. Argue that under any distribution $\mathbb P$ which has a density we get $\|f-g\|=0$ for the $\mathcal L_2(\mathbb P)$ norm.
:::

The problem in this exercise is that the $\mathcal L_2$ distance "ignores" pointwise differences (in fact any countable number of them). Therefore in order to get our vector space to work the same way as $\mathbb R^d$ we need to say that $f$ and $g$ are effectively the same function as far as $\mathcal L_2(\mathbb P)$ is concerned. Formally, we say that the elements (vectors) of $\mathcal L_2(\mathbb P)$ are not functions themselves, but groups of functions that are equivalent to each other for purposes of computing norms or inner products. We call these groups *equivalence classes*. Again, this is not really that critical to understand and doesn't have many implications as far as this book is concerned. Practically speaking, everyone considers *functions* to be the vector elements of $\mathcal L_2$ spaces and 

Now we can finally state our "offical" definition of $\mathcal L_2(\mathbb P)$:

::: Definition
# $\mathcal L_2(\mathbb P)$

Given a distribution $\mathbb P$, The vector space $\mathcal L_2(\mathbb P)$ is the set of equivalence classes of functions $f$ for which the norm $\| f\|^2 = \mathbb P f^2$ is defined and finite. We equip this set with the inner product $\langle f,g \rangle = \mathbb P fg$ which induces the norm.
:::

::: Exercise
Let $\mathbb P(Z)$ be such that $Z ~ \text{Unif}(0,2\pi)$. Calculate the $\mathcal L_2(\mathbb P)$ inner product between $\cos(z)$ and $\sin(z)$. Based on the law of cosines, what does that mean the "angle" is between these functions?
:::
...
::: Exercise
Let a random variable $Z$ and functions $f,g$ be such that $\mathbb E[f(Z)] = \mathbb E[g(Z)]=0$. Use the definitions of epxectation and covarinance to show that $E[f(Z)^2] = \|f\|^2$ and $\mathbb C[f(Z),g(Z)] = \langle f, g \rangle$ where the norm and inner product are with respect to $\mathcal L_2(\mathbb P)$.
:::
...
In a finite-dimensional vector space, if $v$ and $u$ are orthogonal vectors (which we write $u \perp v$) then the length of the vector $v-u$ is $\|v-u\|^2 = \|v\|^2 + \|u\|^2$. Recall that you can think of $v-u$ as the translation to the origin of the "arrow" going from the point $u$ to the point $v$. In $\mathcal L_2$ things work the same way. 

::: Theorem
# General Pythagorean Theorem
In any Hilbert space, $\|f-g\|^2 = \|f\|^2 + \|g\|^2$. 
...
::: Exercise
Let $\mathbb P(Z)$ be such that $Z ~ \text{Unif}(0,10)$. Let $f(z) = 1(0< z<1)$ and let $g(z) = 1(5< z<6)$. Check that $f \perp g$ in $\mathcal L_2(\mathbb P)$ and verify that $\|f-g\|^2 = \|f\|^2 + \|g\|^2$.
:::
...
::: Proof
# Orthogonal projection is well-defined
For the definition above to make sense, technically we have to prove that there *is* a minimum-distance point to $v$ in $\mathcal T$: it could be that there is none (what is the one rational number that is closest to $\pi$?) or that it is not unique (what is the integer that's closest to $1/2$?). For the purposes of this book you can take it as fact that it is well-defined. The proof is not complicated, but it requires more fluency with convergence and infimums than I'd like to assume you have (as well as the parallelogram law). A key required assuption which is satisfied in all cases within this book is that the subspace is *closed*, i.e. that it contains its limits. If you are curious, you can take a screenshot of this definition and ask a large language model to supply the proof.
:::
...
::: Example
Let's presume that $\mathbb P(Z)$ is the uniform distribution over $[0,1]$ and let $\mathcal T$ be the subspace of $\mathcal L_2(\mathbb P)$ comprising of of all linear functions with no intercept, i.e. functions that can be written as $f(z) = cz$ for some slope $c$. You can verify that this is a subspace.
...
The proof of the above is based on the common theme that many interesting vector spaces can be built up as a kind of "sum" of subspaces. 

::: Definition
# Orthogonal Sum and Complement
Two subspaces $\mathcal V_1$ and $\mathcal V_2$ are orthogonal if every vector $v_1 \in \mathcal V_1$ is orthogonal to every vector $v_2 \in \mathcal V_2$. 
...
::: {.Example #example-bounding-integral}
...
$$
|\mathbb E[f(Z)]| = \left|\int f(Z) \cdot 1 \ \text{d}\mathbb P\right| = |\langle f, 1 \rangle| \le \|f\|.
$$
...
Looking at the two examples, you may realize that the uphill direction and the steepness in that direction are *everything* you need to know in order to characterize any linear funtion $f$. The uphill direction can be represented by a unit vector and the steepness is a factor that it is scaled by. We call that scaled unit vector $t_f$, our vector reprenter of $f$. This vector contains all the information there is to know about $f$, and vice versa. Technically they are not the same object, but we can always reconstruct one from the other.
...
The function $f \mapsto \mathbb E_{\mathbb P}[f(Z)]$ is a functional (a linear functional, in fact). The expectation functional is defined on all of $\mathcal L_2(\mathbb P)$ since every square-integrable function is integrable (see Example \ref{example-bounding-integral}).
:::
...
$$
\|h_n\|^2 = \frac{1}{2}\int_{-n^{-1}}^{n^{-1}} 2^2\sqrt{n}^2 \ \text{d}z = 1.
$$ 
On the other hand, 
$$
...
::: Proof
First we'll quickly prove that $T$ is continuous. It's counterintuitive, but linear functionals in infinite dimensions can be discontinuous. In fact, continuity is the same thing as boundedness, but we'll just prove the implication in one direction. Consider a sequence of vectors $v_n \to v$ in $\mathcal V$. Since $T$ is bounded, $|Tv_n-Tv| = |T(v_n-v)| \le \|T\|_\text{op} \|v_n -v\| \to 0$, proving that $T$ is continuous (a sequence of inputs converging to a limit means the sequence of outputs converges to the function evaluated at that limit).
...
Now we'll show that the function $T$ only responds to changes in its input that are aligned with $\tilde t$, just like in the finite-dimensional case. Let $\mathcal T$ be the span of $\tilde t$.  Now let's pick an arbitrary input $v$ and consider the decomposition 
$$
v = \underbrace{(v - \mathcal T v)}_u + \mathcal T v.
$$ 
Now let's calculate the slope of $T$ in the direction of $\tilde t + cu$ as a function of $c$: how much we perturb away from the direction of the steepest slope $\tilde t$:
$$
s(c) = \frac{T(\tilde t + cu)}{\|\tilde t + cu\|}.
$$
Since the slope is maximized in the direction $\tilde t$, we know that $s(c)$ has its maximum at $c=0$, meaning that the derivative is zero there: $s'(0) =0$. However, from the above, we can also calculate the derivative as
$$
s'(c) = \frac{d}{dc}\left( \frac{T\tilde t + cTu}{\sqrt{1 + c^2\|u\|}} \right)
$$
where we've expanded the numerator using linearity and calculated the denominator with the pythagorean theorem ($u \perp \tilde t$ by construction). A tedious application of the product and chain rules and evaluating at $c=0$ gives $s'(0) = T u$. But since we already know that must be zero, we conclude $Tu = 0$. In other words, $T$ does not care about any component of its input that is not aligned with the direction of maximum increase $\tilde t$. 
