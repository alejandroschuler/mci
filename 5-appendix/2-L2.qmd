...
You can check that the usual dot product and the $\mathcal L_2(\mathbb P)$ inner product satisfy these conditions. I'll also use the fact that the norm is induced by the inner product: $\|v\|^2 = \langle v, v \rangle$.
...
The orthogonal *projection* of a vector $v$ onto a subspace $\mathcal T$ is defined to be the vector in $T$ that is closest to $v$. In this book we'll use the notation
$$
...
```{.tikz}
%%| fig-cap: A visual representation of orthogonal projection
...
```tikz
%%| fig-cap: Visual representation of the inner product giving the length of a projection.
\resizebox{13cm}{!}{%
...
We call what's left over when you subtract the projection from the original vector the *residual*. An important geometric property of projections that you may recall from linear algebra (and which I have already been showing in the figures) is that residuals are always orthogonal to the entire subspace that gets projected on.

::: Theorem
# Orthogonality of Residuals
For any subspace $\mathcal T$ and vector $t \in \mathcal T$, $t \perp (v - \mathcal Tv)$.

::: Proof
For any $t\in\mathcal{T}$ set
$$
\phi_t(\epsilon)=\|\,v - \bigl(\mathcal{T}(v)+\epsilon\,t\bigr)\|^2.
$$

The fact that $\mathcal Tv$ is the minimizer of the distance to $v$ means that if we perturb it by $\epsilon$ in any direction $t$, we must be increasing the distance. Therefore for all $t$, $\phi_t(\epsilon)$ is minimized at $\epsilon = 0$. Since $\phi_t$ is a smooth function in $\epsilon$ (a quadratic, in fact) that means if we take the derivative at $\epsilon = 0$ it should be zero.
$$
\phi'(\epsilon)
=\frac{d}{d\epsilon}\langle v-(\mathcal{T}(v)+\epsilon t),\,v-(\mathcal{T}(v)+\epsilon t)\rangle
=-2\langle v-(\mathcal{T}(v)+\epsilon t),\,t\rangle
$$
by expanding the inner product and taking derivatives (linearity). At $\epsilon=0$, setting the above to zero shows
$$
\langle v-\mathcal{T}(v),\,t\rangle=0.
$$
:::
...
``` tikz
%%| fig-cap: Illustration of the triangle inequality.
...
::: callout-tip
If you don't understand (in a precise way) what I mean by convergence of a sequence you should review @def-conv-seq.
:::
...
\begin{tikzpicture}[tdplot_main_coords, scale=1.2]
  ...
  % draw the plane f(v) = t1*v1 + t2*v2
  ...
  % pick a sample vector v = (v1,v2)
  \coordinate (V) at (2,1,0);
  \fill[purple] (V) circle (1.5pt) node[right] {$v$};

  % compute f(v) and draw the vertical up to the plane
  \pgfmathsetmacro{\fv}{\tone*2+\ttwo*1}
  \coordinate (PV) at (2,1,\fv);
  \draw[dashed] (V) -- (PV);

  % mark the point on the plane
  \fill[purple] (PV) circle (1.5pt);

  % --- dotted projection to f-axis ---
  \coordinate (Faxis) at (0,0,\fv);
  \draw[dotted] (PV) -- (Faxis);

  % tick on the f-axis and label
  \draw[line width=1pt] (Faxis) -- ++(0,0.05,0) node[left] {$f(v)$};
...
``` tikz
%%| fig-cap: The same linear function from the top down. Darker blue indicates larger values of $f(v)$. Grey lines are the level sets $f(v) = c$, spaced at equal intervals in $c$ so that the density of those lines indicates the slope.
\resizebox{10cm}{!}{%
...
``` tikz
%%| fig-cap: A different linear function from the top down.
\resizebox{10cm}{!}{%
\begin{tikzpicture}[scale=1.2]
...
$$
Interestingly, however, $T$ does not have a "direction of steepest ascent". To understand this, let's first define the slope of $T$ as the rise in $T$ over a unit change in the input $f$. Equivalently, we may consider the rise in $T$ over an arbitrary run: 
$$
\frac{T(f) - T(f+h)}{\|h\|}. 
$$
Since $T$ is linear, the slope in a given direction is the same at all points so we can take $f=0$.

Now let's pick some directions $h$ (these are functions; recall functions are the vectors in $\mathcal L_2$) and see what the magnitude of the slope $\frac{|Th|}{\|h\|}$ is. Consider the functions (directions, vectors)
...
In the field of functional analysis, functionals that don't have a direction of steepest increase are called *unbounded*. 

::: Definition
# Bounded Linear Functional
A linear functional $T: \mathcal V \to \mathbb R$ is called bounded if
$$
