...
# Real Vector Space
::: {#def-vector-space}
A (real) _vector space_ is a set $\mathcal V$ that satisfies the following properties: 
...
::: {.callout-important appearance="minimal"}
::: {#exr-L2-covarinace}
Let a random variable $Z$ and functions $f,g$ be such that $\mathbb E[f(Z)] = \mathbb E[g(Z)]=0$. Use the definitions of epxectation and covariance to show that $E[f(Z)^2] = \|f\|^2$ and $\mathbb C[f(Z),g(Z)] = \langle f, g \rangle$ where the norm and inner product are with respect to $\mathcal L_2(\mathbb P)$.
:::
:::
