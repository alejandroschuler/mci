...
The very last detail we need to attend to is we need to define what it means for two different (pointwise) functions to be considered "equivalent" in $\mathcal L_2$. This is a very minor detail that you can quickly forget about for most purposes, but I'll go over it here for completeness.
...
You can check that the usual dot product and the $\mathcal L_2(\mathbb P)$ inner product we defined above satisfies these conditions. In what follows below, I'll also use the fact that the norm is induced by the inner product: $\|v\|^2 = \langle v, v \rangle$. As a consequence of this and the definition above, we have the simple corollaries $\|cv\| = c\|v\|$ and $\|v\| = 0 \implies v=0$.
...
::: {#exm-subspace}
Consider the subset of $\mathcal L_2(\mathbb P)$ comprised of all functions that have zero mean under $\mathbb P$, i.e. functions $f$ where $\mathbb E[f(Z)] = 0$. This subset $\mathcal V$ is actually a sub*space* because it is a vector space. To verify that, we just need to check that $\mathcal V$ is a vector space: if we add and scale elements with it, the results are still in $\mathcal V$.
...
This establishes the orthogonality $\langle v - \mathcal{T}v, t \rangle = 0$ for all $t \in \mathcal T$.
:::
####KL HERE
...
It's clear that the dot product between an argument $v$ and some fixed vector $a$ is linear in the argument $v$, but actually the relationship goes both ways: *all* linear functions can be written as an inner (dot) product between the argument and some other fixed vector in the vector space. Formally, for any function $f$ which is linear and maps $\mathbb R^d$ to $\mathbb R$, there is some fixed $t_f \in \mathbb R^d$ that "represents" the function in the sense that $f(v) = \langle t_f, v \rangle$ for all $v \in \mathbb R^d$. We call the vector $t_f$ the "Riesz representer" of the function $f$.
...
Let $Z \sim \text{Unif}(-1,1)$ and let $T: f \mapsto f(0)$ be the functional that takes $f \in \mathcal L_2(\mathbb P)$, evaluates $f$ at the point $z=0$, and returns the result. Clearly, $T$ is linear under the definition of function addition and scaling in $\mathcal L_2$ because 
...
Now, let's calculate the slope of $T$ in the direction of $\tilde t + cu$ as a function of $c$, i.e., how much we perturb away from the direction of the steepest slope $\tilde t$. That is, 
$$
