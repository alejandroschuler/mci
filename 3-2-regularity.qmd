...
Now we turn to @def-regularity. The first step in understanding its meaning is to understand the following standard asymptotic statement which says that $\hat\psi_n = \hat\Psi(\p^{(n)})$ is a $\sqrt{n}$-asymptotically normal estimate of $\psi = \Psi(\p)$: $$
...
::: {#exm-path-convex}
Presume the model is the set of all continuous distributions. Pick any two points $p, p' \in \model$. Then the set of densities $\{p: p = \epsilon p' + (1-\epsilon) p\}$ is a path parameterized by $\eps \in [0,1]$. You can think of this as the straight-line path starting at $p$ (at $\eps=0$) and ending at $p'$ (at $\eps=1$). This is the canonical example of a path, which we will call a *convex path*.
:::
