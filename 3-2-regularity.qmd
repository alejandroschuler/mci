...
Thus, a sensible list of desiderata for an estimator might be:
...
for some function $\phi$ with mean zero ($\mathbb E[\phi(Z)]=0$) and finite variance $\mathbb V[\phi(Z)] < \infty$.
:::
::::
...
Given our desiderata, asymptotically linear (AL) estimators are a very sensible class of estimators to restrict ourselves to. For one, AL estimators are guaranteed to be consistent. This is guaranteed by the fact that any influence function $\phi$ is mean-zero, so the associated sample mean $\mathbb{P}_n\phi$ converges to 0 by the law of large numbers.
...
Constructing asymptotically valid confidence intervals and p-values for any asymptotically linear estimator is therefore as simple as estimating $\sigma^2_* = \mathbb P \phi^2$. If we can construct an estimate $\hat\sigma_*$ that is close to $\sigma_*$ for large-enough $n$, then we can estimate the standard error of our estimate with $\hat\sigma = \frac{\sigma_*}{\sqrt n}$. Additionally, we will get asymptotic 95% coverage of $\psi$ with an interval of the form $[\hat\psi - 2\hat\sigma/\sqrt{n}, \hat\psi + 2\hat\sigma/\sqrt{n}]$ (this is a "Wald-type" confidence interval).
...
::: {.Exm #exm-sample-mean-var-IF}
## Variance of the sample mean

In the case of the sample mean estimator $\hat\psi = \frac{1}{n} \sum Z_i$ we previously saw the influence function was $\phi(z) = z - \psi$. A plug-in estimate of $\mathbb P\phi^2$ is therefore given by $\frac{1}{n} \sum \hat\phi(Z_i)^2 = \frac{1}{n} \sum (Z_i - \hat\phi)^2$.
:::
...
:::: {.callout-important appearance="minimal"}
::: {#exr-unit-score}
Presume a path has a score such that $\dot{\sqrt{p}} = \frac{1}{2} \sqrt{p} s$ has unit norm in $\eltwo$. If we advance a very small amount $\epsilon$ along this path, approximately what is the $\eltwo$ distance between $\sqrt{p_\eps}$ and $\sqrt p$? *Hint: according to @def-score,* $\frac{\sqrt{p_\eps} - \sqrt p}{\eps}$ *is approximately what?*
:::
::::
