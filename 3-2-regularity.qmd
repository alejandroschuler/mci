...
::: {#def-regularity-1}
An estimator $\hat\Psi$ is *regular* in the model $\mathcal P = \{\norm(\psi, 1):\psi \in \mathbb R\}$ if the sampling distribution of 
$$
\sqrt{n}\left(\hat\Psi(\p^{(n)}_n - \Psi\left(\mathrm P_n\right)\right) \rightsquigarrow \mathsf N(0, 1)
$$ 
for the sequence of distributions $\mathbb P_n = \norm(\psi + n^{-1/2}, 1)$.
:::
:::

To understand this definition, let's first understand the following standard asymptotic statment which says that $\hat\psi_n = \hat\Psi(\mathbb P_{\dot n})$ is a $\sqrt{n}$-asymptotically normal estimate of $\psi = \Psi(\mathbb P)$:
$$
\sqrt{n}\left(\hat\Psi(\mathbb P_{\dot n}) - \Psi\left(\mathbb P\right)\right) \rightsquigarrow \norm(0, 1)
$$ 

For each sample size $n$, the left-hand side here is a random variable corresponding to the scaled and centered sampling distribution of the estimator, drawing the $n$ data points from the distribution $\mathbb P$. You can think of standing still at the distribution $\mathbb P$, first drawing samples of size $n=1$ and generating the sampling distribution of $\hat\psi_n$